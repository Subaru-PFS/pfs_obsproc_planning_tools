...
def readUserSample(para,Print=True):
    ...
    if para['mode'] == 'local':
        ...
        df = pd.DataFrame(query.fetchall(), \
                          columns=['ob_code' ,'ra' ,'dec' ,'pmra', 'pmdec', 'parallax', 'epoch' ,'priority' ,'effective_exptime', \
                                   'is_medium_resolution' , "proposal_id", "rank" , "grade"])
    ...
    uSamp['identify_code']=[uSamp['proposal_id'][tt]+uSamp['ob_code'][tt] for tt in range(len(uSamp))]
...
def PPP_centers(sample_f,nPPC,conta,contb,contc,randomseed,mutiPro=True,Print=True,Plot=True):
    ...
    for sample in target_DBSCAN(sample_f,1.38,Print):
        ...
        while any(sample_s['exptime_PPP']>0) and len(peak)<nPPC:     #only for quick testing!!! 
    ...
    for uu in range(len(ppc_g)): #run netflow for each ppc group
        ...
        for i, (vis,  tel) in enumerate(zip(res,  telescope)):
            ...
            if len(vis)==0:
                ...
                tot_weight=1/sum(sample[np.in1d(sample['identify_code'],obj_allo_id)]['weight'])
...
def netflowAssign(sample, point_l):
    ...
    for ppc in point_l_pri:
        lst=np.where(np.in1d(sample['identify_code'],ppc['allocated_targets'])==True)[0]
...
def complete_ppc(sample, mode):  
    ...
    if mode=="compOFtgt_weighted":
        ...
        #patrly observed
        index_part=np.where((sample['exptime_PPP'] > sample['Netflow_time']) & (sample['Netflow_time']>0))[0]
...
def iter1(samp,weight_initialGuess,nppc_l,nppc_m,commode,randomSeed,TraCollision,printTF):    
    ...
    best_weight=opt.fmin(fun2opt,weight_initialGuess,xtol=0.01,ftol=0.01,\
...
def output(obj_allo_tot, uS_tot, dirName='.'):
    ...
    obList=Table([ob_code,ob_obj_id, ob_ra,ob_dec,ob_pmra,ob_pmdec,ob_parallax,ob_equinox,ob_priority,ob_exptime,ob_resolution,proposal_id,\
                  proposal_rank,ob_weight_best,ob_allocate_time_netflow, ob_ppc_code],\
...
def plotCR(cR, sub, obj_allo, dirName='.', show_plots=False): 
    ...
    plt.text((len(cR)+1)*0.85,100*np.mean(cR[:,-2]),"{:2.2f}%".format(100*np.mean(cR[:,-2])),color='r',\
             fontsize=12)
    ...
    plt.text(len(fib_eff)*0.85,np.mean(fib_eff),"{:2.2f}%".format(np.mean(fib_eff)),color='r',\
             fontsize=12)
...
def run(readsamp_con,TimeOnSource_l,TimeOnSource_m,iter1_on=False,iter2_on=False,iter3_on=False, dirName='.', show_plots=False):

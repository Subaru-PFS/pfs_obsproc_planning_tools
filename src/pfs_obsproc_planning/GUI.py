...
class GeneratePfsDesignGUI(object):
    ...
    def getfolder_repodir(self):
        fname = QFileDialog.getExistingDirectory()
        self.app_window.lineEdit_repodir_path.setText(fname)
    ...
    def obstime_add(self):
        ...
        if self.app_window.radioButton_classic.isChecked():
            obstime_start = self.app_window.dateTimeEdit_starttime.time().toString("H:mm:ss")
            obstime_stop = self.app_window.dateTimeEdit_stoptime.time().toString("H:mm:ss")
            obstime_combined = obsdate #+ " " + obstime_start + " " + obstime_stop
        else:
            obstime_combined = obsdate
    ...
    def obstime_remove(self):
        for item in self.app_window.listWidget_obstime.selectedItems():
            self.app_window.listWidget_obstime.takeItem(self.app_window.listWidget_obstime.row(item))
    ...
    def load_template_config(self):
        ...
        try:
            with open(filename, "r") as file:
                ...
                self.config_window.plainTextEdit_config.setPlainText(self.config_tem_new)
                self.config_window.show() 
                self.config_window.pushButton_saveconfig.clicked.connect(self.save_template_config)
    ...
    def config_template_rev(self, config_ori):
        ...
        #localPath_ppc
        if self.app_window.box_readPPC.isChecked() and len(self.app_window.lineEdit_ppc_local_path.text()) > 0:
        ...
        for row_temp in range(nrow_dates):
            obs_dates += f'"{self.app_window.listWidget_obstime.item(row_temp).text()}"'
            obs_dates += ", "
        repl_old = "obs_dates="
    ...
    def rungpd(self):
        ...
        logger.info(f"runtime_ppp = {time_ppp:.2f}, runtime_qplan = {time_qplan:.2f}, runtime_sfa = {time_sfa:.2f}")
    ...
    def run(self):
        ...
        self.app_window.pushButton_design_local_classic.clicked.connect(self.getfolder_design)
        ...
        self.app_window.pushButton_repodir.clicked.connect(self.getfolder_repodir)

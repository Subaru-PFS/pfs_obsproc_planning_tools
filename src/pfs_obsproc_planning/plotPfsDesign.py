...
def plot_FoV(df_fib, df_ag, alpha=1.0, title='', fname='', save=True, show=True,
             pa=0.):
    ...
    for f in uafib:
        df_fib.loc[df_fib.fiberId==f, 'pfi_x'] = df_fibId.loc[df_fibId.fiberId==f, 'x']
        df_fib.loc[df_fib.fiberId==f, 'pfi_y'] = df_fibId.loc[df_fibId.fiberId==f, 'y']
    
    # plt.clf()
    fig = plt.figure(num='design_FoV', figsize=(10, 6), dpi=80, 
    ...
    c = {'un': 'slategrey', 'dot':'black',
         'sky': 'deepskyblue', 'fstar': 'green', 'sci': 'salmon',
         'ag': 'blueviolet'}
    c_list = it.cycle(['salmon', 'royalblue', 'orange', 'limegreen', 'violet'])

    s=10.
    ax1.scatter(df_fib[df_fib.targetType==4].pfi_x, df_fib[df_fib.targetType==4].pfi_y, 
                c=c['un'], marker="x", s=s, alpha=alpha, lw=1, label=f'UNASSIGNED ({len(df_fib[df_fib.targetType==4].pfi_y)})')
    ax1.scatter(df_fib[df_fib.targetType==10].pfi_x, df_fib[df_fib.targetType==10].pfi_y, 
                c=c['dot'], marker="o", s=s, alpha=alpha, lw=0, label=f'BLACKSPOT ({len(df_fib[df_fib.targetType==10].pfi_y)})')
    ax1.scatter(df_fib[df_fib.targetType==2].pfi_x, df_fib[df_fib.targetType==2].pfi_y, 
                c=c['sky'], marker="o", s=s, alpha=alpha, lw=0, label=f'SKY ({len(df_fib[df_fib.targetType==2].pfi_y)})')
    ax1.scatter(df_fib[df_fib.targetType==3].pfi_x, df_fib[df_fib.targetType==3].pfi_y, 
                c=c['fstar'], marker="o", s=s*2.0, alpha=alpha, lw=0, label=f'FLUXSTD ({len(df_fib[df_fib.targetType==3].pfi_y)})')
    ax1.scatter(df_fib[df_fib.targetType==1].pfi_x, df_fib[df_fib.targetType==1].pfi_y, 
                c=c['sci'], marker="o", s=s, alpha=alpha, lw=0, label=f'SCIENCE ({len(df_fib[df_fib.targetType==1].pfi_y)})')
    ax1.scatter(df_ag.ag_pfi_x, df_ag.ag_pfi_y,
                c=c['ag'], marker='s', s=s, alpha=alpha, lw=0, label='guidestar')
    for i in range(1,7):
        ang = np.deg2rad(-60*(i-1)-9)
        agn=len(df_ag[df_ag.camId==(i-1)].ag_pfi_x)
        ax1.text(245*np.cos(ang),245*np.sin(ang), f'{i} ({agn})', ha='center', fontsize=10)
    ...
    line1 = patches.FancyArrow(r*np.cos(-1*np.pi), r*np.sin(-1*np.pi), 2*r*np.cos(0.), 2*r*np.sin(0.), head_width=0, color='grey')
    line2 = patches.FancyArrow(r*np.cos(-2*np.pi/3), r*np.sin(-2*np.pi/3), 2*r*np.cos(np.pi/3), 2*r*np.sin(np.pi/3), head_width=0, color='grey')
    line3 = patches.FancyArrow(r*np.cos(2*np.pi/3), r*np.sin(2*np.pi/3), 2*r*np.cos(-1*np.pi/3), 2*r*np.sin(-1*np.pi/3), head_width=0, color='grey')
    ax1.add_patch(line1)
    ax1.add_patch(line2)
    ax1.add_patch(line3)
    ...
    arr_e = patches.FancyArrow(posne[0], posne[1], dl*de[0], dl*de[1], color='dimgrey', width=3.)
    arr_n = patches.FancyArrow(posne[0], posne[1], dl*dn[0], dl*dn[1], color='dimgrey', width=3.)

    ax1.add_patch(arr_e)
    ax1.add_patch(arr_n)
    ax1.text(posne[0]+dl*2.5*de[0], posne[1]+dl*2.5*de[1], f'E',
             ha='center', va='center', fontsize=10, color='dimgrey')
    ax1.text(posne[0]+dl*2.5*dn[0], posne[1]+dl*2.5*dn[1], f'N',
             ha='center', va='center', fontsize=10, color='dimgrey')
    ...
    label_per_prog = []
    ax2.hist(df_fib[df_fib.targetType==3]['psfMag'], bins=bins, range=(mmin, mmax),
             color=c['fstar'], lw=0, alpha=0.4, label='Std star')
    for i, p in enumerate(proposals):
        mags = df_fib[(df_fib.targetType==1)&(df_fib.proposalId==p)]['psfMag'].values
        n_mags = len(mags)
    ...
    ax2.hist(mag_per_prog.T, bins=bins, range=(mmin, mmax), histtype='barstacked',
             color=c_sci, lw=0, alpha=0.4, label=label_per_prog, stacked=True)
    ...
    ax3.hist(df_ag['agMag'], bins=bins, range=(mmin, mmax),
             color=c['ag'], lw=0, alpha=0.4)
...
def init_check_design():
    ...
    df = pd.DataFrame(data=None,
                      columns=['sky_mean', 'sky_std', 'sky_min', 'sky_max', 'sky_sum',
...
def check_design(designId, df_fib, df_ag):
    ...
    df_ch = pd.DataFrame(data=np.append(check_fibers(df_fib), check_ags(df_ag)).reshape(1,17),
                         columns=['sky_mean', 'sky_std', 'sky_min', 'sky_max', 'sky_sum',
                                  'std_mean', 'std_std', 'std_min', 'std_max', 'std_sum',
                                  'ag1', 'ag2', 'ag3', 'ag4', 'ag5', 'ag6', 'ag_sum'],)
...
def check_fibers(df_fib):
    ...
    # sky
    try:
        arr = df_fib[df_fib['targetType']==2].groupby(by='sector')['targetType'].count().values
        f.extend((np.mean(arr), np.std(arr), np.min(arr), np.max(arr), np.sum(arr)))
        if(arr.size < 12): f[2]=0.
    except ValueError:
        f.extend((np.nan, np.nan, np.nan, np.nan, np.nan))
    # std
    try:
        arr = df_fib[df_fib['targetType']==3].groupby(by='sector')['targetType'].count().values
        f.extend((np.mean(arr), np.std(arr), np.min(arr), np.max(arr), np.sum(arr)))
        if(arr.size < 12): f[7]=0.
    except ValueError:
        f.extend((np.nan, np.nan, np.nan, np.nan, np.nan))
...
def get_field_sector(df_fib):
    ...
    t = np.rad2deg(np.arctan2(df_fib['pfi_y'], df_fib['pfi_x'])) + 180.
...
def calc_nedirection(pa):
    ...
    ne = np.array([[0., 1.],
                   [-1., 0.]])
    a = np.deg2rad(pa)
    rot = np.array([[np.cos(a), -1.*np.sin(a)],
                   [np.sin(a), np.cos(a)]])

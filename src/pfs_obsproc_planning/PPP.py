...
def readUserSample(para, Print=True):
    ...
    if para["mode"] == "local":
        ...
        df["resolution"] = ["M" if v is True else "L" for v in df["resolution"]]
...
def target_collision(sample, sep=2 / 3600.0, Print=True):
    ...
    if len(labels_c) == 0:
        ...
        # index = list(np.where(np.in1d(labels, labels_c) is False)[0]) + [
        #     np.where(np.in1d(labels, kk) is True)[0][0] for kk in labels_c
        # ]
        return sorted(index)
...
def PFS_FoV(ppc_ra, ppc_dec, PA, sample, mode=None):
    ...
    # index_ = np.where(polygon.contains_points(point) is True)[0]
...
def PPP_centers(
    ...
    for sample in target_DBSCAN(sample_f, 1.38, Print):
        ...
        while any(sample_s["exptime_PPP"] > 0) and (
            len(peak) < nPPC
            ...
            index_ = PFS_FoV(peak_x, peak_y, 0, sample_s)

            if len(index_) > 0:
                # ppc_id,ppc_ra,ppc_dec,ppc_PA=0
                peak.append([len(peak), peak_x, peak_y, 0])
                ...
                # ppc_id,ppc_ra,ppc_dec,ppc_PA=0
                peak.append([len(peak), peak_x_t, peak_y_t, 0])

            # -------------------------------
            if len(index_) > Nfiber:
                index_ = random.sample(list(index_), Nfiber)

            # targets in the PPC observed with 900 sec
            sample_s["exptime_PPP"][list(index_)] -= 900
...
def netflowAssign(sample, point_l):
    ...
    for ppc in point_l_pri:
        lst = np.where(
            # np.in1d(sample["identify_code"], ppc["allocated_targets"]) is True
            np.in1d(sample["identify_code"], ppc["allocated_targets"])
        )[0]
